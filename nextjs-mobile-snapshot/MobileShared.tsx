...
function usePhotoLike(photoId: string) {
  ...
  const toggle = async () => {
    ...
    if (SUPABASE_CONFIGURED && authUser?.id) {
      const supabase = getSupabaseBrowserClient();
      if (next) {
        await supabase.from('prototype_likes').upsert({ user_id: authUser.id, photo_id: photoId });
      } else {
        await supabase.from('prototype_likes').delete().eq('user_id', authUser.id).eq('photo_id', photoId);
      }
    }
  };
