...
// Temporarily override MIN_PHOTOS for testing so we can see output even with few photos!
// We cast to any to bypass the 'readonly' modifier for this quick test script.
(PULSE_V5_HOF as any).MIN_PHOTOS = 1; 
...
async function run() {
  ...
  for (const r of results) {
    if (r.photoCount === 0) continue;
    const badge = photographerBadge({ hofScore: r.hofScore, accountAgeDays: r.item.accountAgeDays });
    md += `| ${i++} | @${r.item.username} | ${r.photoCount} | ${r.avgScore.toFixed(1)} | **${r.hofScore ? r.hofScore.toFixed(1) : '-'}** | ${badge || '-'} |\n`;
  }
