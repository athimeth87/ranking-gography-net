...
ออกแบบ **ทุกหน้า ทุก state ทุก interaction** ของระบบนี้ — ตั้งแต่ landing page ไปจนถึง admin dashboard
...
7. **Admin UI** ที่ functional + ใช้ minimal (ไม่ต้องสวยเท่า public)

### Format ที่อยากได้
- Figma file (preferred) หรือ
- Detailed mockups + HTML/CSS prototype หรือ
- Tailwind-based React components ที่เอาไปใช้ได้เลย
...
- กระชับแต่อบอุ่น — "เลือกภาพให้ฤดูกาล" (ไม่ใช่ "vote now!")
- หลีกเลี่ยง gamification language ("level up", "achievement unlocked") — ดู casual
- Editorial tone — "ภาพให้ฤดูกาล Spring 2026" — เหมือน art magazine
- Error messages = ขออภัย + ระบุปัญหา + วิธีแก้

### ตัวอย่าง copy

**Hero:** "ภาพถ่ายที่เล่าเรื่อง — โดยช่างภาพและนักเดินทาง"
**Empty state (no photos):** "ยังไม่มีภาพในหมวดนี้ — เป็นคนแรกที่อัปโหลด"
...
ใน decisions.md ของ project มี Round 2 (R1-R8) ที่ founder ยังไม่ tick — บางคำถามส่งผลกับ design:
...
**Start:** อ่าน `design-system.md` ก่อน — แล้ว tackle pages ตามลำดับใน list ด้านบน
**Ask questions ถ้าไม่ชัด** — better to clarify than assume
**Deliver in chunks** — ไม่ต้องรอจนทั้งหมด ส่ง draft ทีละ section ได้
